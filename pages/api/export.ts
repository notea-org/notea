...
import TreeActions, {ROOT_ID, HierarchicalTreeItemModel} from 'libs/shared/tree';
...
export default api()
    ...
    .get(async (req, res) => {
        ...
        async function addItem(item: HierarchicalTreeItemModel, prefix: string = ""): Promise<void> {
            const note = await req.state.store.getObjectAndMeta(getPathNoteById(item.id));
            ...
            const uniquePath = duplicate[basePath] ? `${basePath} (${duplicate[basePath]})` : basePath;
        ...
        if (rootItem) {
            await Promise.all(
                rootItem.children.map((v) => addItem(v))
            );
        }
